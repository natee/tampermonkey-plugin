(function () {
  ...
  // ==UserScript==
  // @name           小说阅读脚本（Safari阅读模式）
  // @name:zh-CN     小说阅读脚本（Safari阅读模式）
  // @version        0.1
  // @namespace      kerncink@gmail.com
  ...
  // @include        *://www.bhzw.cc/book/*/*/*.html
  // @include        *://www.60355.com/*/*/
  ...
  // 这里是自己加的
  // @include        *://www.sczprc.com/*
  // @include        *://www.pfwx.com/*
  // end 自己增加的
  ...
  const sites = [
    // 详细版规则示例。注：该网站已无法访问。
    {
      ...
      url: "^https?://www\\.test\\.net/Html/\\S+\\.shtm$",          // // 站点正则... (~~必须~~)
    ...
    {
      siteName: "冰火中文",
      url: /^https?:\/\/www\.bhzw\.cc\/book\/[\d\/]+\.html$/,
      titleReg: /(.*?)最新章节,(.*?)-.*/,
      fixImage: true,
      contentSelector: "#ChapterContents",
      contentRemove: 'font[color="red"]',
      contentReplace: {
        "&lt;冰火#中文.*|冰火中文&nbsp;(www.)?bhzw.cc(?:【首发】|)|冰.火.中文|绿色小说|lvsexs|冰火中文": "",
    ...
    // ================== 采用 iframe 方式获取的 ====================
    {
      ...
      contentReplace: "看小说“就爱读书”|bhzw|www\\.92to\\.com",
      useiframe: true,
    ...
    {
      siteName: "笔趣读",
      url: /^https?:\/\/www\.biqudu\.com\.tw\/\d+_\d+\/\d+.html/,
      siteExample: 'http://www.biqudu.com.tw/17_17768/8280656.html',
      contentSelector: "#content"
    },
    {
      siteName: "笔趣看",
  ...
  var Rule = {
    ...
    contentSelectors: [".article-con", "#pagecontent", "#contentbox", "#bmsy_content", "#bookpartinfo", "#htmlContent",
  ...
  var tpl_mainHtml = `
    <div class="novel">
      ...
      <div class="novel-loading"></div>

      <div class="novel-conf__box">
        <div class="novel-skins"></div>
      </div>
    </div>
  `;
  ...
  var tpl_mainCss = `
    ...
    .novel-skins__item.active{
      border: 2px solid #1ed63473;
    }
    ...
    @media screen and (min-width: 0px) {
      ...
      .novel-content article { padding-left: 16px; padding-right: 70px; }
    }
    
    @media screen and (min-width: 375px) {
      /* iPhone 6 in portrait */
      .novel-content article { padding-left: 18px; padding-right: 70px; }
    }
    
    @media screen and (min-width: 414px) {
      /* iPhone 6 Plus in portrait */
      .novel-content article { padding-left: 20px; padding-right: 70px; }
    }
    ...
    @media screen and (min-width: 667px) {
      ...
      .novel-content article { padding-left: 40px; padding-right: 70px; }
    }
    
    @media screen and (min-width: 704px) {
      /* iPad in landscape with the sidebar open */
      .novel-content article { padding-left: 42px; padding-right: 42px; }
    }
    
    @media screen and (min-width: 736px) {
      /* iPhone 6 Plus in landscape */
      .novel-content article { padding-left: 60px; padding-right: 60px; }
    }
  ...
  var App = {
    ...
    launch: function () {
      // 只解析一次，防止多次重复解析一个页面
      if (document.body && document.body.getAttribute("name") == "readMode") {
        return App.toggle();
      }
      ...
      if (hasContent) {
        document.body.setAttribute("name", "readMode");
    ...
    processPage: function (parser) {
      ...
      // cache vars
      App.$doc = $(document);
      App.$content = App.$doc.find(".novel-content");
      App.$loading = App.$doc.find(".novel-loading");
    ...
    initDocument: function (parser) {
      ...
      document.body.innerHTML = $.nano(tpl_mainHtml, parser);
    },
    clean: function () {
      $('body > *:not(".novel, .novel-trigger, .noRemove, iframe[name=\'mynovelreader-iframe\']")').remove();
    ...
    appendPage: function (parser, isFirst) {
      ...
      if (chapter.length && parser.isSection) { // 每次获取的不是一章，而是一节
        ...
        chapter = $("<article>")
          .attr("class", "novel-content--" + App.pageNum)
          .append(
        ...
        App.activeUrl = parser.curPageUrl;
    ...
    scroll: function () {
      ...
      // 距离底部多少高度400（px）开始加载下一页
      if (!App.paused && !App.working && App.getRemain() < 400) {
        App.scrollForce();
      }
    ...
    updateCurFocusElement: function () { // 滚动激活章节列表
      ...
      if (App.lastId !== id) {
        ...
        // App.activeUrl = App.curPageUrl || App.prevUrl;
      }
    },
    getRemain: function () {
    ...
    doRequest: function () {
      ...
      if (nextUrl && !App.isTheEnd && !(nextUrl in App.parsedPages)) {
        ...
        App.$loading
          ...
          .append("<a href='" + nextUrl + "' title='点击打开下一页链接'>正在载入下一页" + (useiframe ? "(iframe)" : "") + "...</a>");
    ...
    addNextPage: function (parser) {
      if (parser.content) {
        ...
        var msg = (parser.isTheEnd == 'vip') ?
          ...
          '错误：没有找到下一页的内容。';
        App.$loading.html(
          '<a href="' + App.curPageUrl + '">' + msg + '点此打开下一页。</a>')
          .show();
      }
  ...
  var skinCss = `
    .novel.novel-skin__white {
      background-color: #E2E2E2;
    }
    
    .novel.novel-skin__white .novel-content {
      color: rgb(27, 27, 27);
      background-color: white;
      box-shadow: 0px 6px 12px 3px rgba(0, 0, 0, 0.2);
    }
    
    .novel.novel-skin__white .novel-content .title{
      border-bottom: 1px solid rgb(220, 220, 220);
    }
    
    .novel.novel-skin__sepia {
      background-color: #DCD2C1;
    }
    
    .novel.novel-skin__sepia .novel-content {
      color: rgb(79, 50, 28);
      background-color: rgb(248, 241, 227);
      box-shadow: 0px 6px 12px 3px rgba(0, 0, 0, 0.2);
    }
    .novel.novel-skin__sepia .novel-content .title{
      border-bottom: 1px solid rgb(230, 218, 201);
    }
    
    .novel.novel-skin__gray {
      background-color: #2C2C2C;
    }
    .novel.novel-skin__gray .novel-content {
      color: rgba(255, 255, 255, 0.78);
      background-color: rgb(74, 74, 77);
      box-shadow: 0px 6px 12px 3px rgba(0, 0, 0, 0.24);
    }
    .novel.novel-skin__gray .novel-content .title{
      border-bottom: 1px solid rgb(111, 111, 111);
    }
    
    .novel.novel-skin__dark {
      background-color: #040404;
    }
    .novel.novel-skin__dark .novel-content {
      color: rgb(176, 176, 176);
      background-color: rgb(18, 18, 18);
      outline: 1px solid #272727;
    }
    .novel.novel-skin__dark .novel-content .title{
  ...
  var UI = {
    ...
    skins: {
      "novel-skin__white": {
        name: "白底黑字",
        val: "novel-skin__white",
        textColor: "#1A1A1A",
        bgColor: "#FFFFFF"
      },
      "novel-skin__sepia": {
        name: "乌贼墨色",
        val: "novel-skin__sepia",
        textColor: "rgb(79, 50, 28)",
        bgColor: "rgb(248, 241, 227)"
      },
      "novel-skin__gray": {
        name: "灰色",
        val: "novel-skin__gray",
        textColor: "rgba(255, 255, 255, 0.78)",
        bgColor: "rgb(74, 74, 77)"
      },
      "novel-skin__dark": {
        name: "夜间模式",
        val: "novel-skin__dark",
        textColor: "color: rgb(176, 176, 176)",
    ...
    refreshMainStyle: function () {
      ...
      UI.$mainStyle = $('<style id="main_style">')
        .text(mainCss)
        .appendTo('head');
    },
    refreshSkinStyle: function (skinName, isFirst) {
      // $(".novel").removeClass().addClass('novel', skinName);
      document.querySelector('.novel').className = 'novel ' + skinName;
      $('.novel-skins__item').removeClass('active');
    ...
    preferencesUI: function (event) {

      const $confWrap = $(".novel-conf__box");
  ...
  var Setting = {
    ...
    get skin_name() {
      return GM_getValue("skin_name") || "novel-skin__white";
    },
    set skin_name(val) {
